...
def _build_protoc_command(target, ctx):
    ...
    protoc_command += " --plugin=\"protoc-gen-grpc=%s\"" % (ctx.executable._protoc_gen_grpc.path)
...
def ts_proto_library_aspect_(target, ctx):
    ...
    tools.extend(ctx.files._protoc_gen_grpc)
...
ts_proto_library_aspect = aspect(
    ...
    attrs = {
        "_change_import_style": attr.label(
            ...
            cfg = "host",
            allow_files = True,
            default = Label("//src/bazel/ts_proto_library:change_import_style"),
        ),
        "_protoc": attr.label(
...
def _ts_proto_library_impl(ctx):
    ...
    return struct(
        ...
        providers = [
            DefaultInfo(files = outputs, runfiles = ctx.runfiles(transitive_files = es5_srcs)),
            DeclarationInfo(
                declarations = dts_outputs,
                transitive_declarations = transitive_declarations,
                type_blocklisted_declarations = depset([]),
